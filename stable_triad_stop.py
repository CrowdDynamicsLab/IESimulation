...
def plot_heat_map(data, title, min, sc, ho):
    ...
    title_save = 'figures/heatmaps_stopping/' + title + '.png'
    #plt.show()
...
stable_triads_array = np.zeros((len(sc), len(ho)))
...
for i in sc:
    ...
    for j in ho:
        ...
        stable_triads_array[int((1-sc_likelihood)/float(sc[1])), int(ho_likelihood/float(ho[1]))] = summary_stats[9]
...
plot_heat_map(apl_array, 'Avg APL', 0, sc, ho)
plot_heat_map(cluster_coeff_array, 'Avg Cluster Coeff', 0, sc, ho)
plot_heat_map(stable_triads_array, 'Avg Final Stable Triad Count', 0, sc, ho)
